...
| config.OBOT_ARTIFACT_AZURE_CLIENT_SECRET | string | `""` | Azure client secret for published workflow storage when using explicit Azure credentials. |
...
| config.OBOT_ARTIFACT_GCS_SERVICE_ACCOUNT_JSON | string | `""` | Inline GCS service account JSON for published workflow storage. If empty, Application Default Credentials are used. |
| config.OBOT_ARTIFACT_S3_ACCESS_KEY_ID | string | `""` | Access key ID for published workflow storage when explicit S3-compatible credentials are required. |
...
| config.OBOT_ARTIFACT_S3_SECRET_ACCESS_KEY | string | `""` | Secret access key for published workflow storage when explicit S3-compatible credentials are required. |
